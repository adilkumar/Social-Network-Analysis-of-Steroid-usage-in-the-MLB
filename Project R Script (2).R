...
model_worldseries <- lm(WAR ~ average_Jaccard, data=totalnetwork_4)
summary(model_worldseries)

temp <- totalnetwork_4[!is.na(totalnetwork_4$`Last year`),]
cor(temp[,c(2:7,10:19)])

# Predict WAR using network statistics
model_eigen <- lm(WAR ~ deg_in + betw + close + eigen.cent, data=totalnetwork_4)
summary(model_eigen)
